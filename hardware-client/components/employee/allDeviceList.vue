<template>
  <v-container class="main-container">
    ...
    <v-row class="device-header-container" align="center">
      <v-col>
        <h3 class="title-heading">Device List :</h3>
      </v-col>
    ...
    <v-row class="table-container">
      ...
      >
        <template v-slot:item.newRepair="{ item }">
          <!-- Show select field only when device.assignedTo is empty -->
          <v-select
            v-if="item.deviceStatus === 'new'"
            v-model="selectedOption"
            @change="redirectAccordinglyToPages(item)"
            :items="repairOptions"
            label="New Repair"
          ></v-select>
        </template>
...
export default {
  data() {
    return {
      ...
      item:null,
      allAllotments: [],
      repairOptions: ["New Device", "repair Device", "Temporary Device"],
      headers: [
  ...
  watch: {
    ...
    selectedOption(newVal, oldVal) {
      // Watch for changes in selectedRepair and redirect based on its value
      if (newVal === "New Device") {
        // Redirect to a new page or perform any other action
        this.$router.push(
          `/admin/newDeviceRequest/${this.selectedDevice?.deviceId}`
        );
      } else if (newVal === "repair Device") {
        this.$router.push(
          `/admin/addNewDeviceRepair/${this.selectedDevice?.deviceId}`
        );
      } else {
        this.$router.push(
          `/admin/newDeviceRequestTemporary/${this.selectedDevice?.deviceId}`
        );
      }
      // You can add more conditions based on different values of selectedRepair
    },
  },
  mounted() {
  ...
  methods: {
    ...
    redirectAccordinglyToPages(item){
      this.$nextTick(() => {
    this.item = item;

    if (this.selectedOption === "New Device") {
      // Redirect to a new page or perform any other action
      this.$router.push(`/admin/newDeviceRequest/${this.item?.deviceId}`);
    } else if (this.selectedOption === "repair Device") {
      this.$router.push(`/admin/addNewDeviceRepair/${this.item?.deviceId}`);
    } else {
      this.$router.push(`/admin/newDeviceRequestTemporary/${this.item?.deviceId}`);
    }
  });
    },
